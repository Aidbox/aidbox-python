...
if username is None: raise Exception("AIDBOX_CLIENT_USERNAME environment variable is missing")
if password is None: raise Exception("AIDBOX_CLIENT_PASSWORD environment variable is missing")
...
class API(BaseModel):
    ...
    def delete(self):
        ...
        response = requests.delete(
            url=f"{base}/{resource_type}/{self.id}", auth=basic
        )
        response.raise_for_status()  # TODO: handle and type HTTP codes except 200+
